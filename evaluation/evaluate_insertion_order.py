...
def get_cluster_status() -> ClusterStatusResponse:
    with grpc.insecure_channel(CONTROLLER_TARGET) as channel:
        stub = project2_pb2_grpc.ControllerServiceStub(channel)
        return stub.ClusterStatus(ClusterStatusRequest())        


def print_cluster_status(resp : ClusterStatusResponse):
    print("\n" + "=" * 80)
...
def main() -> None:
    ...
    # Step 4: evaluate cluster status/partition layout
    cluster_status_resp : ClusterStatusResponse = get_cluster_status()
